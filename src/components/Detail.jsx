...
const Detail = () => {
  ...
  return (
    <>
      {results.length > 0 ? (
        results.map((person) => (
          <Grid item xs={14} sm={12} md={8} key={person._id}>
            {" "}
            {/* Using person._id as key */}
            <Card>
              <Grid container>
                <Grid item xs={7}>
                  <CardContent sx={{ padding: 4 }}>
                    ...
                    <Typography variant="body2">
                      ...
                      {/* Ensure Aadhaar is displayed correctly */}
                    </Typography>
                    <Typography variant="body2">
                      Reported On:{" "}
                    ...
                    <Typography variant="body2">
                      ...
                      >
                        Reward: ₹{parseFloat(person.reward).toFixed(2)}{" "}
                        {/* Parse reward as a float */}
                      </Typography>
                    </Typography>
                  </CardContent>
                </Grid>
              ...
              <Box display="flex" justifyContent="flex-end" p={3}>
                <Button
                  ...
                  onClick={() =>
                    (window.location.href = `/person/${person._id}`)
                  } // Use person._id for the URL
                >
                  More Details
                </Button>
                <Button
